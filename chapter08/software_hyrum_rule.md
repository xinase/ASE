...
说个真事。我的团队以前开发了一个 Form Recognizer（智能表单）服务，客户传进来一张发票、单据或者表格的照片，我们拿 AI 模型识别出上面的各种语义字段，比如总价、签名、付款方信息，同时把这些字段在图片上的像素坐标也一并返回。老版本返回的坐标是基于原始图像坐标系的——图片要是侧着放的，坐标自然也是歪的，但我们就这么返回了。新版本自动把图片摆正，坐标也基于摆正后的图像重新计算，我们心想这回客户总该满意了吧。结果上线不到四十八小时，一个大客户就发紧急工单来了，说他们的发票流水线全崩了。一查才发现，客户基于我们上一版 API 的 "不靠谱" 行为，自己摸索写了一套脚本，处理了部分侧着放的图片，但这脚本却在新版 API 中崩溃了。

如果说 Form Recognizer 的"惊喜"还只是影响了某个大客户，那软件史上还有更极端的例子——一个日期计算的错误，因为被上亿份电子表格（Excel）依赖，三十年来无人敢碰。这些案例背后有一条定律，来自 **Hyrum K. Wright**：
...
另外，不得不提 Javascript 仓促设计造成的各种包袱，尽管大家都说 “我以后有时间会改进的”， 但是这些陈年老bug 会永远存在于互联网上 —— 例如，typeof null === "object" 这个公认的 Bug 至今无法修复，因为全球数以亿计的网页早已把它当作 "基准事实" 来依赖。
...
在 Excel 中直接输入日期 1900-2-29（或将数字 60 设置为日期格式），Excel 会错误地将其显示为 1900年2月29日；而输入 =DATE(1900,2,28) 的数字序号是 59，=DATE(1900,3,1) 的数字序号是 61。这个“第 60 天”的幽灵日期导致 1900 年 3 月 1 日之前的所有日期序列号全错了一天。
![Excel 闰年日期 bug](image-23.png)

无数人指出过这个 bug，微软也心知肚明，但**没有人计划修复**——因为修复了这个 bug 的 Excel 发布后，全球所有依赖日期计算的财务报表将集体偏移，无数利率计算、合同到期日判断将出现细微而违规的偏差，市场上会出现无数的老版本 Excel 和新版本 Excel 的冲突，这将导致很多企业用户干脆弃用 Excel。

Lotus/Excel 的闰年日期 Bug 之所以改不了，**根本原因不是技术，而是经济学**。修正一行代码只需五分钟，但要修正它引发的海量数据资产的合规性校验，成本无限趋近于无穷大。当修复成本高于 Bug 造成的实际损害时，**Bug 就变成了 Feature**。
...
Raymond Chen 在《The Old New Thing》里写了几十年 Windows 开发的故事，核心主题就是当一个系统被几亿人用的时候，你永远想不到他们会依赖什么隐性行为。他在书中为"Backward Compatibility"贡献了一整章的案例（下图只是案例的一小部分）：

![alt text](image-21.png)

Raymond (以及知名IT 创业者 Joel Spolsky) 记录过一个经典案子：Windows 3.x 版的《模拟城市》（SimCity）有一个"释放内存后继续读取"（Use-After-Free）的严重 Bug。在旧版系统里这个 Bug 没触发崩溃，但到了 Windows 95，全新的 32 位内存分配器让游戏一开就死机。微软工程师没有逼游戏厂商打补丁，而是在 Windows 95 里写了专门的代码：**只要检测到运行的是 SimCity，就自动将内存分配器切换到不立即释放内存的特殊兼容模式。**
...
但"老版本已经够好"这个判断，在不同用户群体中有完全不同的含义。

用户光谱的一端是技术爱好者——他们订阅每个预览版，为每一次 UI 调整欢呼，变化本身就是他们使用软件的理由之一。另一端是实务工作者——他们用软件来完成某项具体工作，对版本号毫无兴趣，只要求"它像昨天一样正常工作"。

软件厂商的升级策略，往往默认了"所有用户都是技术爱好者"这个假设。release notes 写满了"性能提升""全新架构"——这些语言取悦了光谱左端，却让光谱右端付出了"习惯断裂"的代价。而光谱右端，往往是付费主体。

这也是为什么"good enough"在不同用户眼里如此不同：对技术爱好者来说，最新版才是"good"；对实务工作者来说，现在这个能用的版本，就是"good enough"。而厂商每发一个新版，都在重新定义"好"的标准——同时也在重新消耗"够用"那部分用户的耐心。

对厂商来说，真正的挑战不是"让所有人升级"，而是区分"谁值得被频繁打扰"。

对技术爱好者：提供预览版、beta 通道、社区反馈机制——让他们参与演进，成为品牌的布道者。

对实务工作者：提供 LTS（长期支持）版本，承诺安全补丁但不改变 UI 和 API 行为——让他们在熟悉的环境中高效工作。

不是所有用户都需要"最新"，也不是所有用户都抗拒"变化"。好的产品策略，是让技术爱好者找到新鲜感，同时让实务工作者找到安全感。 这两个群体的需求不是对立的，只是需要被不同的交付渠道所满足。
...
一个大厂成熟产品的团队日常就是开发和维护的组合，维护占了越来越多的比重：
1. 正在设计下一个大版本 V3；  
...
"老版本是有生命的。" 当软件被使用后，它就逐步嵌入到了用户的工作流和用户的文化里，用户围绕它搭建的附属生态，反过来向新版本提出了硬性约束条件。有的还成为了用户的日常语言的一部分， 例如用户表达搜索信息时的 “you should google it", "问一下度娘”。 
