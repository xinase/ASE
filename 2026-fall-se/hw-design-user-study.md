# 智能硬件开发 · 物理原型与真实用户调研实操指南（含《构建之法》第四版章节推荐）
...
- **C (Competitors 竞争)**：列出具体的现有竞品（必须包含“用户选择不使用任何相关产品”的现状分析），说明本产品实现从 0 到 1 转变的差异化壁垒。
- **D (Delivery & Data 交付与验证数据)**：说明 11 周内将硬件实体送达目标用户手中的具体渠道，并明确列出用于评估用户反馈的量化指标（如 90 天持续在线率、任务完成时长、净推荐值 NPS 等）。
...
物理原型不是“穷人的原型”，而是一种有意识的设计策略——用最低成本锁定产品最核心的物理约束和交互直觉，让后续所有的技术开发和工程投入都围绕一个已经被验证的目标展开。

原型的目标是以最低成本建立实体的尺寸、结构和交互流程，供用户实际操作。原型不要求自研电路完全跑通或最终代码就绪。
...
- 他还会把多种按键图案打印出来粘在木头表面，模拟不同设计界面

这个“预型”的目的**不是测试 PalmPilot 能不能做得出来**，而是测试**人们到底会如何使用、多频繁地使用它**。霍金斯通过这段经历收集到的关键数据包括：
...
霍金斯后来在回忆这段经历时说：“人们觉得我疯了，但我对它将如何工作有了感觉。”这个木块预型最终证明了：他本人真的很愿意拥有这样一款设备，且主要使用四个功能：通讯录、日历、备忘录和任务清单。这个简单的试验提供的证据，**足以指引并证明投入更多资金开发真正原型和最终产品的合理性**。
...
**误区**：有些团队会不自觉地追求“像真机”：屏幕要亮、按键要响、外壳要光滑。结果成本上去了，迭代速度下来了，用户反而不敢提意见——“你们做得这么认真，我说不好是不是不太合适？”
