...
1. 你认为你们项目有多复杂？
2. 你的产品原型和用户调查的照片/视频；
3. 当目标用户使用产品原型时，他们的反应如何，和你原来设想的 NABCD 中的 N（需求），B（好处）一致么？你从中学到了什么？

## 0. 复杂度自评

上周很多团队和个人都写了项目的 NABCD，在进一步细化之前，我们可以对照《构建之法》第四版第 8 章关于复杂度的分类，分析自己的项目复杂度，记住，我们要在剩余的 9 周时间把产品做出来，不能拖延。

- **简单**：需求明确、技术成熟、边界清晰、可预测。做出来就知道对不对，比如一个固定功能的传感器模块，或者重复一个市面上主流智能硬件的基本功能。
- **繁杂**：需求可分解、组件多但关系可枚举、有成熟方法论。可以拆成很多独立部分，每部分都有成熟解法，比如一个多传感器数据采集系统，或者在市面上成熟的智能硬件的基础上，开发有限的几个扩展功能。
- **复杂**：需求动态变化、组件间非线性耦合、结果不可完全预测。各部分互相影响，用户期待、行为、环境、硬件、软件的各种因素交织。
- **混乱**：需求模糊、边界不清、因果关系不明确、无先例可循。连“要解决什么问题”都说不清，比如“做一个更懂用户的全新架构、全新组合的智能硬件”。
...
如果团队无法就复杂度等级达成共识，说明 NABCD 还没想清楚，要多探索多讨论，keep it simple，聚焦。像软件工程老手的告诫：**不要试图同时爬两棵树**。

**复杂度自评是整份指南的认知基调。** 它决定了 NABCD 的边界、原型策略和调研强度。在后文的 NABCD、原型制作、调研对象分层、困难清单和预防清单中，都会反复呼应这个分类。
...
**复杂度越高，原型越要早做、越要频繁接触用户。** 简单/繁杂项目可以在方案相对确定后再做原型；复杂/混乱项目必须在方案还很粗糙时就用纸壳原型验证方向。
...
**复杂度越高，调研对象越要分层。** 简单/繁杂项目可以聚焦少数典型用户；复杂/混乱项目必须覆盖不同角色、不同偏好、不同环境，才能逼近真实需求。
...
**如果团队连复杂度等级都达不成共识，NABCD 更不可能达成共识。复杂度自评是 NABCD 的第一道门。**
...
**误区**：产品经理（或调研人员）对自己的方案充满热情，滔滔不绝介绍功能多好、解决了什么问题。用户被感染，纷纷点头，说“会用”。参见《构建之法》第八章的案例：

![ user study](image/hw-design-user-study/1790147111613.png)

**建议**：热情留给团队内部，调研现场必须中立。调研人员与产品介绍人分离。介绍人可以热情，主持人必须中立。

**不问“你会用吗”，只问“你现在怎么做”。** 问过去的行为，不问未来的意愿。意愿是廉价的，行为是真实的。

**观察离场反应。** 案例中最大的信息量，不在会议室里，在用户走出会议室之后的耸肩、摇头。如果条件允许，记录用户离开调研现场后的自然言行。

**不把点头当结论。** 用户点头，不等于用户需要。用户说“会用”，不等于用户真的会用。
**用户的反应并不都是真实的。** 产品经理的热情本身就是一种引导。他滔滔不绝的介绍，不是中立的呈现，而是推销。用户被感染、点头、说“会用”，不是因为真的需要，而是因为不好意思当面否定一个热情的人。

> 参见困难 13：创新者“做太多”也会污染调研。

### 困难 6：把“挺好的”当真反馈

**误区**：中国用户尤其容易给面子。问他“这个设计怎么样”，他说“挺好的”。追问“有没有什么问题”，他说“没有没有，也讲不出什么”。
...
困难 12 和 13 是同一问题的两个侧面：困难 12 是“教用户”，困难 13 是“吓用户”。共同解法是：**创新者退后，用户向前。**
